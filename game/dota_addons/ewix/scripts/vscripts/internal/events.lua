...
function Ewix:_OnGameRulesStateChange(keys)
  ...
  elseif newState == DOTA_GAMERULES_STATE_HERO_SELECTION then
    Ewix:PostLoadPrecache()
    Ewix:OnAllPlayersLoaded()
  ...
  elseif newState == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
    Ewix:OnGameInProgress()
  end
end

-- An NPC has spawned somewhere in game.  This includes heroes
function Ewix:_OnNPCSpawned(keys)
  local npc = EntIndexToHScript(keys.entindex)

  if npc:IsRealHero() and npc.bFirstSpawned == nil then
    npc.bFirstSpawned = true
    Ewix:OnHeroInGame(npc)
  end
end

-- An entity died
function Ewix:_OnEntityKilled( keys )
...
function Ewix:_OnConnectFull(keys)
  Ewix:_CaptureEwix()
